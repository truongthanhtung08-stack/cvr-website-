    ...
    <>
      ...
      <main className="flex-1">
        ...
        <section className="relative isolate flex min-h-[420px] items-end overflow-hidden">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-cl-ink via-cl-ink/60 to-black/40" />
          <div className="relative mx-auto w-full max-w-5xl px-4 pb-10 pt-28 sm:px-6">
            <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.28em] text-cl-gold sm:text-xs">{l.eyebrow}</p>
            ...
            <Link href={l.ctaHref} className="mt-6 inline-flex items-center gap-2 rounded-lg bg-cl-gold px-6 py-3 text-sm font-semibold text-cl-ink transition hover:bg-cl-gold-soft active:scale-95">
              {l.ctaLabel}
        ...
        <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
          ...
          <div className="mt-10 grid grid-cols-2 gap-4 lg:grid-cols-4">
            {l.stats.map((s) => (
              <div key={s.label} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-center">
                <p className="font-serif text-2xl font-bold text-cl-gold-soft sm:text-3xl">{s.value}</p>
          ...
          <div className="mt-10 grid grid-cols-1 gap-5 md:grid-cols-3">
            {l.blocks.map((b, i) => (
              <div key={b.title} className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
                <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-cl-gold/15 font-serif text-lg font-bold text-cl-gold-soft">{i + 1}</span>
          ...
          {/* CTA cuối */}
          <div className="mt-12 flex flex-col items-center gap-4 rounded-3xl border border-white/10 bg-gradient-to-br from-cl-charcoal to-cl-ink p-8 text-center sm:p-12">
            ...
            <div className="mt-2 flex flex-wrap items-center justify-center gap-3">
              <Link href={l.ctaHref} className="rounded-lg bg-white px-6 py-3 text-sm font-semibold text-cl-ink transition hover:bg-white/90">{l.ctaLabel}</Link>
