    ...
    <>
      ...
      <main className="flex-1">
        <div className="mx-auto max-w-7xl px-4 pb-20 pt-24 sm:px-6 lg:px-8">
          ...
          <h1 className="mt-8 font-serif text-2xl font-bold text-white sm:text-3xl">Dự án nổi bật</h1>
          <p className="mt-1.5 text-sm text-white/55">Các dự án căn hộ, khu đô thị và nghỉ dưỡng đáng chú ý tại Miền Trung.</p>
            ...
            {projects.map((p) => (
              ...
              >
                ...
                <div className="relative aspect-[4/3] w-32 shrink-0 overflow-hidden rounded-xl sm:w-60 md:w-72">
                  ...
                  <span className="absolute left-2 top-2 rounded-md bg-cl-gold px-2 py-0.5 text-[11px] font-bold uppercase tracking-wide text-cl-ink">{p.status}</span>
                </div>
                {/* Thông tin phải */}
                <div className="flex min-w-0 flex-1 flex-col py-1 pr-1">
                  <div className="flex items-start justify-between gap-3">
                    <h3 className="line-clamp-2 font-semibold leading-snug text-white transition-colors group-hover:text-cl-gold-soft sm:text-lg">{p.name}</h3>
                    <span className="shrink-0 text-base font-medium text-white sm:text-xl">{p.priceFrom}</span>
                  </div>
                  ...
                  <p className="mt-1.5 line-clamp-2 text-xs leading-relaxed text-white/55">{p.type} · Chủ đầu tư {p.developer}. {p.overview[0]}</p>
                  <div className="mt-auto flex flex-wrap items-center gap-1.5 pt-3">
                    {p.amenities.slice(0, 3).map((a) => (
                      <span key={a} className="rounded-md border border-white/12 bg-white/5 px-2 py-0.5 text-[11px] text-white/65">{a}</span>
                    ))}
                  </div>
                </div>
              </Link>
