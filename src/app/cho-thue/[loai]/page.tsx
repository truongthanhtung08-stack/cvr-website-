...
// ============================================================================
// /cho-thue/<slug> phục vụ HAI loại trang, phân biệt bằng chính slug:
//   · slug là LOẠI HÌNH (can-ho-chung-cu, nha-rieng…) → trang danh mục loại hình
//   · slug là KHU VỰC   (da-nang, hue, quang-ngai…)   → nhà đất cho thuê theo tỉnh
// Gộp vào một route để đường dẫn ngắn, sạch: /cho-thue/da-nang (chứ không phải
// /cho-thue/khu-vuc/da-nang) — đúng cách Batdongsan/Homedy làm; ngắn thì khách
// nhớ được và Google cũng chuộng hơn.
//
// Đây là nhóm từ khoá ĐÔNG NHẤT của ngành ("cho thuê nhà Đà Nẵng", "thuê căn hộ Huế"),
...
export async function generateMetadata({ params }: { params: Promise<{ loai: string }> }): Promise<Metadata> {
  ...
  const soTin = demTinTheoKhuVuc(listings.filter((l) => (l.purpose ?? "ban") === "thue"), kv.name);

  const title = tieuDeKhuVuc("thue", kv.name);
  const description = moTaKhuVuc("thue", kv.name, soTin);
  return {
    title,
    description,
    alternates: { canonical: `/cho-thue/${kv.slug}` },
    openGraph: { title, description, url: `/cho-thue/${kv.slug}`, type: "website" },
...
export default async function DanhMucChoThuePage({ params }: { params: Promise<{ loai: string }> }) {
  ...
  // ── TRANG KHU VỰC ─────────────────────────────────────────────────────────
  if (kv) {
    const tinTrongTinh = tinThue.filter((l) => normalizeVi(tinhCua(l.location)) === normalizeVi(kv.name));
    return (
      <>
        <ListingListJsonLd items={tinTrongTinh} heading={tieuDeKhuVuc("thue", kv.name)} path={`/cho-thue/${kv.slug}`} />
        <Header />
  ...
  return (
    <>
      ...
      <main className="flex-1 bg-white">
        ...
        <KhuVucLinks base="/cho-thue" demTheoTinh={demTheoTinh} />
      </main>
      <Footer />
    </>
  );
