...
function StoreButton({ store, href = "#", compact = false }: { store: "apple" | "google"; href?: string; compact?: boolean }) {
  ...
  return (
    <a
      ...
      // BỀ RỘNG CỐ ĐỊNH → 2 nút App Store / Google Play luôn BẰNG NHAU tuyệt đối
      // (trước đây co theo độ dài chữ nên nút Google Play dài hơn), và NGẮN hơn
      // nút CTA phía trên cho gọn.
      className={`flex items-center rounded-xl bg-cvr-ink text-white shadow-sm transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:-translate-y-0.5 hover:bg-black ${compact ? "min-h-[34px] w-[118px] max-w-full gap-1.5 px-2.5" : "min-h-[46px] w-[152px] gap-2.5 px-3.5 py-2"}`}
    >
      {store === "apple" ? (
      ...
      {compact ? (
        <span className="min-w-0 flex-1 whitespace-nowrap text-left text-[11px] font-semibold text-white">
