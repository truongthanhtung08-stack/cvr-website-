        ...
        <div className="relative overflow-hidden bg-[#0b0b0d] shadow-lux">
          ...
          <div className="grid h-[210px] grid-cols-[1fr_164px] items-center overflow-hidden md:hidden">
            ...
            <div className="relative h-full overflow-hidden">
              <Image
                src={asset("/images/app-front-v2.png")}
                alt="Ứng dụng COASTAL LAND trên iPhone"
                width={378}
                height={866}
                sizes="40vw"
                priority
                className="absolute right-0 top-0 h-auto w-[164px] max-w-none"
              />
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      <span className="flex flex-col leading-tight text-left">
        <span className={`${compact ? "text-[9px]" : "text-[10px]"} ${DARK_FAINT}`}>Tải về trên</span>
        <span className={`${compact ? "text-[12px]" : "text-[13px]"} font-semibold text-white`}>{store === "apple" ? "App Store" : "Google Play"}</span>
      </span>
    </a>
  );
}
